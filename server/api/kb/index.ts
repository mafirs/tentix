...
async function generateGeneralKnowledgeRecallIndexes(
  ...
  const promptText = [
    ...
    "3) 对故障排查类知识，用户首问现象和诊断证据必须分开：内存快满、CPU 高、端口未监听、服务未启动、资源不足、OOM 默认属于诊断证据，不作为索引。",
    ...
    "- 重启断开：用户会说“一直重启 / 自动重启 / 反复重启 / 用着用着断开 / 总是断开”，通常不会先说明资源指标。",
    ...
    "8) 对账号、计费、备案、发票类知识，索引应保留用户实际关心的动作或困惑。",
    "9) 对错误日志类知识，可把最核心的错误原文作为一条索引，但不要扩写成解决方案标题。",
    "",
    "禁止作为索引的内容：",
    "- 根因或诊断分支：例如“服务未启动”“端口未监听”“自定义域名未备案”“随机域名”。",
    "- 对故障排查类知识，禁止把触发场景枚举作为索引：例如“运行大模型”“执行构建任务”“启动服务后”“安装依赖时”“批量处理数据时”。",
    "- 对故障排查类知识，禁止把诊断证据或监控指标作为索引：例如“内存快满了”“CPU 长期较高”“内存 80%-90%”“资源监控截图”。",
    "- 内部实现或系统视角：例如“Ingress pending”“LoadBalancer”“Service ExternalIP”。",
    "- 排查动作：例如“ss -tnlp 检查端口监听”“查看网络配置”“收集截图”。",
    "- 客服流程或话术：例如“先确认用户使用的是随机域名还是自定义域名”。",
    "- 完整答案：例如“因为服务未启动导致公网地址准备中”。",
