...
export const issueCanonical = tentix.table(
  "issue_canonical",
  {
    id: serial("id").primaryKey().notNull(),
    title: varchar("title", { length: 254 }).notNull(),
    summary: text("summary").notNull(),
    primaryModule: varchar("primary_module", { length: 50 }).default("").notNull(),
  ...
  (table) => [
    index("idx_issue_canonical_status").on(table.status),
    index("idx_issue_canonical_last_seen_week").on(table.lastSeenWeek.desc()),
    index("idx_issue_canonical_primary_module").on(table.primaryModule),
  ],
);

export const issueClusterRun = tentix.table(
  "issue_cluster_run",
  {
    id: serial("id").primaryKey().notNull(),
  ...
  (table) => [
    index("idx_issue_cluster_run_type_week").on(
      table.runType,
      table.weekStart.desc(),
    ),
    index("idx_issue_cluster_run_status").on(table.status),
    index("idx_issue_cluster_run_run_at").on(table.runAt.desc()),
  ],
);

export const weeklyIssueCluster = tentix.table(
  "weekly_issue_cluster",
  {
    id: serial("id").primaryKey().notNull(),
    ...
    runAt: timestamp("run_at", {
      ...
      .defaultNow()
      .notNull(),
    canonicalId: integer("canonical_id").references(() => issueCanonical.id),
    stableKey: varchar("stable_key", { length: 254 }).notNull(),
    summary: text("summary").notNull(),
    count: integer("count").notNull(),
    ticketIds: char("ticket_ids", { length: 13 }).array().notNull(),
    representativeTicketId: char("representative_ticket_id", { length: 13 }).notNull(),
  ...
  (table) => [
    index("idx_weekly_issue_cluster_run_id").on(table.runId),
    index("idx_weekly_issue_cluster_week_start").on(table.weekStart.desc()),
    index("idx_weekly_issue_cluster_canonical_id").on(table.canonicalId),
    index("idx_weekly_issue_cluster_week_canonical").on(
      table.weekStart,
      table.canonicalId,
    ),
    index("idx_weekly_issue_cluster_run_type_week").on(
      table.runType,
      table.weekStart.desc(),
    ),
  ],
